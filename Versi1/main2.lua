...
do
    ...
    -- Weather quick-select buttons (replaces dropdown)
    do
        local ButtonContainer = Instance.new("Frame")
        ButtonContainer.Name = "WeatherButtons"
        ButtonContainer.BackgroundColor3 = Color3.fromRGB(50,50,50)
        ButtonContainer.BorderSizePixel = 0
        ButtonContainer.Size = UDim2.new(1,0,0,120)
        ButtonContainer.Parent = WeatherTab.TabContent or WeatherTab -- depending on library structure
        ...
        grid.CellSize = UDim2.new(0, math.floor((400)/4), 0, 30) -- approximate; will adjust automatically
        grid.CellPadding = UDim2.new(0,6,0,6)
        grid.FillDirection = Enum.FillDirection.Horizontal
        grid.SortOrder = Enum.SortOrder.LayoutOrder
        grid.Parent = ButtonContainer

        -- Resize container based on rows
        grid.Changed:Connect(function(prop)
            if prop == "AbsoluteContentSize" then
                local rows = math.ceil(#WeatherOptions / 4)
                ButtonContainer.Size = UDim2.new(1,0,0, rows * (30+6) + 16) -- rows * (cellHeight+padding) + vertical padding
            end
        end)
        ...
        for _, name in ipairs(WeatherOptions) do
            ...
            b.MouseButton1Click:Connect(function()
                ...
                for _, child in ipairs(ButtonContainer:GetChildren()) do
                    if child:IsA("TextButton") then
                        styleButton(child, child.Name == selectedWeather)
                    end
                end
                updateStatus("Selected: "..selectedWeather)
                -- immediate purchase once
