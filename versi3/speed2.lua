...
local XSAN_CONFIG = {
    ...
    presets = {
        ...
        Ultra = {purpose = "maximum earnings", autosell = 15},
...
local RandomSpotTab = Window:CreateTab("RANDOM SPOT", 4483362458) -- New Random Spot tab
...
task.spawn(function()
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        elseif not gameProcessed and input.KeyCode == Enum.KeyCode.LeftShift then
            -- Left Shift for Speed Boost toggle
            if speedHackEnabled then
                disableSpeedHack()
            else
                enableSpeedHack("CFrame", 3.0) -- Quick 3x speed boost
                NotifySuccess("Speed Hotkey", "⚡ Left Shift: Speed boost activated!\n🚀 3x speed enabled")
            end
        elseif not gameProcessed and input.KeyCode == Enum.KeyCode.LeftControl then
            -- Left Ctrl for Emergency Stop
            disableSpeedHack()
            setWalkSpeed(defaultWalkspeed)
            NotifyInfo("Emergency", "🛑 Ctrl: Emergency speed stop!\n✅ All speed systems disabled")
        elseif not gameProcessed and input.KeyCode == Enum.KeyCode.Tab then
            -- Tab for Speed Preset Cycle
            local currentPresets = {"Normal", "Fast Walk", "Speed Boost", "Super Speed", "Ultra Speed"}
            local currentIndex = 1
            
            -- Find current preset index
            for i, preset in ipairs(currentPresets) do
                local presetData = SpeedPresets[preset]
                if math.abs(currentWalkspeed - presetData.walkspeed) < 5 and speedHackEnabled == presetData.speedhack then
                    currentIndex = i
                    break
                end
            end
            ...
            currentIndex = (currentIndex % #currentPresets) + 1
            local nextPreset = currentPresets[currentIndex]
            applySpeedPreset(nextPreset)
            NotifySuccess("Speed Cycle", "🔄 Tab: Speed preset cycled!\n🎯 Now using: " .. nextPreset)
        end
    end)
...
local speedHackType = "CFrame" -- "CFrame", "Velocity", "Hybrid"
local speedHackMultiplier = 2.0
local speedHackConnection = nil
local speedHackSession = 0
local bypassDetection = true

-- Smart Fishing Speed Integration
local fishingSpeedBoost = false -- Enable fishing speed boost with speedhack
local originalAutoRecastDelay = 0.5 -- Store original delay
local speedHackFishingMultiplier = 1.0 -- Fishing speed multiplier when speedhack active
local smartFishingEnabled = true -- Smart integration toggle

-- Speed Presets
local SpeedPresets = {
    ["Normal"] = {walkspeed = 16, speedhack = false, multiplier = 1.0},
    ["Fast Walk"] = {walkspeed = 50, speedhack = false, multiplier = 1.0},
    ["Speed Boost"] = {walkspeed = 30, speedhack = true, multiplier = 2.0},
    ["Super Speed"] = {walkspeed = 20, speedhack = true, multiplier = 5.0},
    ["Ultra Speed"] = {walkspeed = 16, speedhack = true, multiplier = 10.0},
    ["Extreme Speed"] = {walkspeed = 16, speedhack = true, multiplier = 20.0},
    ["God Speed"] = {walkspeed = 16, speedhack = true, multiplier = 50.0}
}

-- Traditional WalkSpeed Functions
local function setWalkSpeed(speed)
    local success, error = pcall(function()
        ...
        if humanoid then
            ...
            NotifySuccess("Walk Speed", "🚶‍♂️ Walk speed set to " .. speed .. "\n💡 Using safe Roblox method")
        else
...
-- Advanced SpeedHack Functions
local function startCFrameSpeedHack(multiplier)
    local humanoidRootPart = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not humanoidRootPart then return false end
    
    speedHackSession = speedHackSession + 1
    local mySession = speedHackSession
    
    speedHackConnection = RunService.Heartbeat:Connect(function()
        if speedHackSession ~= mySession or not speedHackEnabled then
        ...
        pcall(function()
            ...
            if humanoid and rootPart and humanoid.MoveVector.Magnitude > 0 then
                local moveDirection = humanoid.MoveVector.Unit
                local speedBoost = moveDirection * (multiplier * 2)
                
                -- Apply bypass techniques if enabled
                if bypassDetection then
                    -- Method 1: Small incremental movements
                    for i = 1, 3 do
                        rootPart.CFrame = rootPart.CFrame + (speedBoost / 3)
                        task.wait(0.001)
                    end
                else
                    -- Method 2: Direct movement
                    rootPart.CFrame = rootPart.CFrame + speedBoost
                end
            end
        end)
    end)
...
local function startVelocitySpeedHack(multiplier)
    local humanoidRootPart = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not humanoidRootPart then return false end
    ...
    speedHackConnection = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            if humanoid and rootPart and humanoid.MoveVector.Magnitude > 0 then
                local moveDirection = humanoid.MoveVector.Unit
                local bodyVelocity = rootPart:FindFirstChild("XSANVelocity")
                
                if not bodyVelocity then
                    bodyVelocity = Instance.new("BodyVelocity")
                    bodyVelocity.Name = "XSANVelocity"
                    bodyVelocity.MaxForce = Vector3.new(4000, 0, 4000)
                    bodyVelocity.Parent = rootPart
                end
                
                bodyVelocity.Velocity = Vector3.new(moveDirection.X * multiplier * 20, 0, moveDirection.Z * multiplier * 20)
            else
                local bodyVelocity = humanoidRootPart:FindFirstChild("XSANVelocity")
...
local function startHybridSpeedHack(multiplier)
    local humanoidRootPart = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not humanoidRootPart then return false end
    
    speedHackSession = speedHackSession + 1
    local mySession = speedHackSession
    local useVelocity = true
    ...
    speedHackConnection = RunService.Heartbeat:Connect(function()
        ...
        -- Alternate between methods for better detection bypass
        useVelocity = not useVelocity
        ...
        pcall(function()
            ...
            if humanoid and rootPart and humanoid.MoveVector.Magnitude > 0 then
                ...
                if useVelocity then
                    ...
                    bodyVelocity.Velocity = Vector3.new(moveDirection.X * multiplier * 15, 0, moveDirection.Z * multiplier * 15)
                else
                    -- CFrame method
                    local speedBoost = moveDirection * (multiplier * 1.5)
                    rootPart.CFrame = rootPart.CFrame + speedBoost
                end
            end
        end)
...
local function enableSpeedHack(hackType, multiplier)
    if speedHackEnabled then
        disableSpeedHack()
        task.wait(0.1)
    end
    ...
    speedHackType = hackType
    speedHackMultiplier = multiplier
    
    local success = false
    
    if hackType == "CFrame" then
        success = startCFrameSpeedHack(multiplier)
    elseif hackType == "Velocity" then
        success = startVelocitySpeedHack(multiplier)
    elseif hackType == "Hybrid" then
        success = startHybridSpeedHack(multiplier)
    end
    
    -- Smart Fishing Speed Integration
    if success and smartFishingEnabled then
        applyFishingSpeedBoost(multiplier)
    end
    
    if success then
        local fishingInfo = ""
        if smartFishingEnabled and fishingSpeedBoost then
            fishingInfo = "\n🎣 Fishing Speed: " .. string.format("%.1f", speedHackFishingMultiplier) .. "x faster"
        end
        
        NotifySuccess("SpeedHack", "⚡ SpeedHack ENABLED!\n\n🚀 Type: " .. hackType .. "\n⚡ Movement: " .. multiplier .. "x" .. fishingInfo .. "\n🔒 Anti-Detection: " .. (bypassDetection and "ON" or "OFF") .. "\n\n⚠️ Use responsibly!")
    else
        speedHackEnabled = false
        NotifyError("SpeedHack", "❌ Failed to enable SpeedHack!\n💡 Make sure your character is spawned")
    end
...
local function disableSpeedHack()
    ...
    -- Reset fishing speed boost
    if fishingSpeedBoost then
        removeFishingSpeedBoost()
    end
    ...
    NotifyInfo("SpeedHack", "🛑 SpeedHack disabled!\n✅ Back to normal speed\n🎮 Manual control restored\n🎣 Fishing speed normalized")
end

local function applySpeedPreset(presetName)
    local preset = SpeedPresets[presetName]
    ...
    -- Apply walkspeed
    setWalkSpeed(preset.walkspeed)
    
    -- Apply speedhack if needed
    if preset.speedhack then
        enableSpeedHack("CFrame", preset.multiplier)
    end
    
    local message = "🎯 " .. presetName .. " ACTIVATED!\n\n🚶‍♂️ WalkSpeed: " .. preset.walkspeed
    if preset.speedhack then
        message = message .. "\n⚡ SpeedHack: " .. preset.multiplier .. "x multiplier"
    end
    message = message .. "\n\n💡 Perfect for " .. (presetName == "Normal" and "safe gameplay" or presetName == "Fast Walk" and "quick movement" or presetName == "Speed Boost" and "fast exploration" or presetName == "Super Speed" and "rapid travel" or "extreme speed needs")
    
    NotifySuccess("Speed Preset", message)
end
...
-- Apply fishing speed boost when speedhack is enabled
local function applyFishingSpeedBoost(speedMultiplier)
    if not smartFishingEnabled then return end
    
    -- Calculate fishing speed multiplier (capped for safety)
    speedHackFishingMultiplier = math.min(speedMultiplier * 0.5, 3.0) -- Max 3x fishing speed
    
    -- Store original delay if not already stored
    if not fishingSpeedBoost then
        originalAutoRecastDelay = autoRecastDelay
    end
    
    -- Apply faster recast delay
    autoRecastDelay = math.max(0.1, originalAutoRecastDelay / speedHackFishingMultiplier)
    fishingSpeedBoost = true
    
    print("XSAN: Fishing speed boost applied - " .. string.format("%.1f", speedHackFishingMultiplier) .. "x faster")
end

-- Remove fishing speed boost
local function removeFishingSpeedBoost()
    if not fishingSpeedBoost then return end
    
    -- Restore original delay
    autoRecastDelay = originalAutoRecastDelay
    fishingSpeedBoost = false
    speedHackFishingMultiplier = 1.0
    
    print("XSAN: Fishing speed boost removed - back to normal")
end

-- Smart speed adjustment for different activities
local function getContextualSpeedMultiplier()
    -- If auto fishing is active, use safer speed
    if autofish then
        return math.min(speedHackMultiplier, 5.0) -- Max 5x when fishing
    end
    
    -- Full speed when not fishing
    return speedHackMultiplier
end

-- Enhanced speed preset system with fishing integration
local function applySpeedPresetEnhanced(presetName)
    local preset = SpeedPresets[presetName]
    if not preset then return end
    ...
    if preset.speedhack then
        message = message .. "\n⚡ SpeedHack: " .. preset.multiplier .. "x multiplier"
        if smartFishingEnabled then
            local fishingSpeed = math.min(preset.multiplier * 0.5, 3.0)
            message = message .. "\n🎣 Fishing Speed: " .. string.format("%.1f", fishingSpeed) .. "x faster"
        end
    end
    message = message .. "\n\n💡 Perfect for " .. (presetName == "Normal" and "safe gameplay" or presetName == "Fast Walk" and "quick movement" or presetName == "Speed Boost" and "fast exploration" or presetName == "Super Speed" and "rapid travel" or "extreme speed needs")
...
print("XSAN: Advanced Speed system initialized!")

-- ═══════════════════════════════════════════════════════════════
-- ENHANCED SPEEDHACK WITH FISHING INTEGRATION
-- ═══════════════════════════════════════════════════════════════

-- Override applySpeedPreset to use enhanced version
applySpeedPreset = applySpeedPresetEnhanced
...
TeleportLocations.RandomSpots = {
    ["🏝️ SISYPUS"] = CFrame.new(-3710.64, -98.77, -956.60),
...
AnalyticsTab:CreateButton({
    ...
    Callback = CreateSafeCallback(function()
        ...
        local stats = string.format("XSAN Ultimate Analytics:\n\nSession Time: %.1f minutes\nFish Caught: %d\nFish/Hour: %d\n\n=== CASTING STATS ===\nMode: %s\nPerfect Casts: %d (%.1f%%)\nNormal Casts: %d\nTotal Casts: %d\n\n=== EARNINGS ===\nItems Sold: %d\nEstimated Ultra: %d coins\nActive Preset: %s", 
            sessionTime, fishCaught, fishPerHour, castingMode, perfectCasts, perfectEfficiency, normalCasts, totalCasts, itemsSold, estimatedUltra, currentPreset
        )
...
-- ═══════════════════════════════════════════════════════════════
-- UTILITY TAB - System Management & Advanced Features + SpeedHack
-- ═══════════════════════════════════════════════════════════════
...
UtilityTab:CreateParagraph({
    ...
    Content = "Kontrol kecepatan dengan WalkSpeed aman atau SpeedHack untuk kecepatan ekstrem. Gunakan dengan bijak!"
})
...
UtilityTab:CreateButton({
    Name = "⚡ Extreme Speed",
    Callback = CreateSafeCallback(function()
        applySpeedPreset("Extreme Speed")
    end, "speed_extreme")
})
...
UtilityTab:CreateButton({
    Name = "🌟 God Speed",
    Callback = CreateSafeCallback(function()
        applySpeedPreset("God Speed")
    end, "speed_god")
})

-- Manual Controls
UtilityTab:CreateParagraph({
    Title = "⚙️ Manual Speed Control",
    Content = "Kontrol manual untuk fine-tuning kecepatan sesuai kebutuhan."
})
...
UtilityTab:CreateSlider({
    Name = "SpeedHack Multiplier",
    Range = {1.0, 100.0},
    Increment = 0.5,
    CurrentValue = speedHackMultiplier,
    Callback = function(value)
        speedHackMultiplier = value
        if speedHackEnabled then
            -- Restart with new multiplier
            local currentType = speedHackType
            disableSpeedHack()
            task.wait(0.1)
            enableSpeedHack(currentType, value)
        end
    end
})
...
UtilityTab:CreateDropdown({
    Name = "SpeedHack Type",
    Options = {"CFrame", "Velocity", "Hybrid"},
    CurrentOption = "CFrame",
    Callback = function(option)
        speedHackType = option
        if speedHackEnabled then
            -- Restart with new type
            local currentMultiplier = speedHackMultiplier
            disableSpeedHack()
            task.wait(0.1)
            enableSpeedHack(option, currentMultiplier)
        end
    end
})
...
UtilityTab:CreateToggle({
    ...
    CurrentValue = bypassDetection,
    Callback = function(value)
        bypassDetection = value
        NotifyInfo("Anti-Detection", "🔒 Anti-Detection " .. (value and "ENABLED" or "DISABLED") .. "\n\n" .. (value and "✅ Uses bypass techniques\n💡 Safer but slightly slower" or "❌ Direct speed method\n⚠️ Faster but more detectable"))
    end
})
...
UtilityTab:CreateToggle({
    ...
    Callback = function(value)
        if value then
            enableSpeedHack(speedHackType, speedHackMultiplier)
        else
            disableSpeedHack()
        end
    end
...
UtilityTab:CreateParagraph({
    ...
    Content = "Kontrol darurat untuk mengatasi masalah atau reset sistem kecepatan."
})
...
UtilityTab:CreateButton({
    ...
    Callback = CreateSafeCallback(function()
        disableSpeedHack()
        setWalkSpeed(defaultWalkspeed)
        NotifySuccess("Emergency Stop", "🛑 ALL SPEED SYSTEMS STOPPED!\n\n✅ SpeedHack: Disabled\n✅ WalkSpeed: Reset to 16\n✅ Safe state restored\n\n💡 You can now move normally")
    end, "emergency_stop")
})

UtilityTab:CreateButton({
    Name = "📊 Speed System Info",
    Callback = CreateSafeCallback(function()
        local info = "🚀 XSAN SPEED SYSTEM STATUS:\n\n"
        info = info .. "🚶‍♂️ Current WalkSpeed: " .. currentWalkspeed .. "\n"
        info = info .. "⚡ SpeedHack: " .. (speedHackEnabled and "ENABLED" or "DISABLED") .. "\n"
        if speedHackEnabled then
            info = info .. "🔧 Type: " .. speedHackType .. "\n"
            info = info .. "📈 Multiplier: " .. speedHackMultiplier .. "x\n"
        end
        info = info .. "🔒 Anti-Detection: " .. (bypassDetection and "ON" or "OFF") .. "\n\n"
        info = info .. "📋 AVAILABLE METHODS:\n"
        info = info .. "• WalkSpeed: Safe Roblox method\n"
        info = info .. "• CFrame: Position manipulation\n"
        info = info .. "• Velocity: Physics-based speed\n"
        info = info .. "• Hybrid: Alternating methods\n\n"
        info = info .. "⚠️ Remember: Higher speeds = higher detection risk!"
        
        NotifyInfo("Speed System", info)
    end, "speed_info")
})
...
UtilityTab:CreateParagraph({
    ...
    Content = "Kontrol karakter dan sistem reset untuk gameplay yang optimal."
})

UtilityTab:CreateToggle({
    Name = "🚶‍♂️ Enable WalkSpeed",
...
UtilityTab:CreateButton({
    Name = "💊 Fix Character Stuck",
    Callback = CreateSafeCallback(function()
        pcall(function()
            local humanoidRootPart = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if humanoidRootPart then
                -- Method 1: Reset position
                humanoidRootPart.CFrame = humanoidRootPart.CFrame + Vector3.new(0, 5, 0)
                
                -- Method 2: Clean up any stuck objects
                local bodyVelocity = humanoidRootPart:FindFirstChild("XSANVelocity")
                if bodyVelocity then
                    bodyVelocity:Destroy()
                end
                
                -- Method 3: Reset physics
                humanoidRootPart.Velocity = Vector3.new(0, 0, 0)
                humanoidRootPart.AngularVelocity = Vector3.new(0, 0, 0)
                
                NotifySuccess("Character Fix", "💊 Character unstuck!\n\n✅ Position reset\n✅ Physics cleared\n✅ Movement restored\n\n🎮 You should be able to move normally now")
            else
                NotifyError("Character Fix", "❌ Character not found!")
            end
        end)
    end, "fix_stuck")
})
